...
#pragma warning disable RS2008 // Enable analyzer release tracking
    static readonly DiagnosticDescriptor DuplicateIdRule = new("EM001", "Duplicate Event ID detected",
        "The Event ID '{0}' is already used by method '{1}'. Event IDs must be unique across the project.",
        "Naming", DiagnosticSeverity.Error, true);
#pragma warning restore RS2008 // Enable analyzer release tracking
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var loggerMessages = context.SyntaxProvider.ForAttributeWithMetadataName(
            ...
            (context, _) => CreateLoggerMessage(context));
        ...
        context.RegisterSourceOutput(loggerMessagesWithOptions, (context, loggerMessagesWithOptions) =>
        {
            ...
            {
                if (seenIds.TryGetValue(loggerMessage.Id, out var existingMethod))
                {
                    context.ReportDiagnostic(Diagnostic.Create(DuplicateIdRule, loggerMessage.Location, loggerMessage.Id, existingMethod.Name));
                    hasDuplicates = true;
                }
                else
                {
    ...
    static LoggerMessage CreateLoggerMessage(GeneratorAttributeSyntaxContext ctx)
    {
        var id = (ushort)0;
        var level = EventLevel.Informational;
        var message = ctx.TargetSymbol.Name;
        ...
        foreach (var arg in attribute.NamedArguments)
        {
            ...
            {
                case "EventId":
                    id = (ushort)(int)(arg.Value.Value ?? 0);
                    break;
                case "Message":
                    message = arg.Value.Value?.ToString() ?? message;
                    break;
                case "Level":
                    ...
                    level = (int)(arg.Value.Value ?? 6) switch
                    {
                        ...
                        _ => throw new InvalidOperationException($"Invalid log level: {arg.Value.Value}")
                    };
                    break;
            }
        }
        ...
        {
            id = (ushort)(attribute.ConstructorArguments[0].Value ?? 0);
        }

        // Decouple the location details from the syntax to allow caching and avoid holding onto syntax trees.
        var syntaxLocation = ctx.TargetNode.GetLocation();
        return new(id, ctx.TargetSymbol.Name, level, message,
